...
#          + generalized Faraday law (RM * lambda^RM_idx instead of lambda^2)  #
# =============================================================================#
import bilby
import numpy as np
...
def model(pDict, lamSqArr_m2):
    ...
    quArr = pArr * np.exp( 2j * (np.radians(pDict["psi0_deg"]) +
                     pDict["RM_radm2"] * np.power(np.sqrt(lamSqArr_m2), pDict['RM_idx']))  )
...
prior_config = {
    ...
    "RM_idx": dict(
        ...
        name="RM_idx", latex_label=r"RM$_{\rm idx}$",
    ),
}
